...
public class Main {
	...
	static class MyNumber implements Comparable<MyNumber> {
		...
		public MyNumber(int idx, int num) {
			super();
			this.idx = idx;
			this.num = num;
		}
	...
	public static void main(String[] args) throws IOException {
		...
		int minNumOfAll = sortedSeq[0].num;
		...
		for (int i = 0; i < size; i++) {
			...
			int minNumOfCycle = cycle.first().num;
			sum -= minNumOfCycle;
			int cnt = cycle.size() - 1;
			int cost1 = sum + minNumOfCycle * cnt;
			
			// 전체에서 가장 작은 원소랑 swap하여 정렬하는 경우
			int cost2 = (sum + minNumOfAll * cnt) + (minNumOfAll * 2 + minNumOfCycle * 2);
