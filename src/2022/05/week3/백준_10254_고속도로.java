...
public class Main {
	static class Answer {
		...
		public Answer(double maxDist, double x1, double y1, double x2, double y2) {
			super();
	...
	public static void main(String[] args) throws IOException {
		...
		for (int tc = 1; tc <= TESTS; tc++) {
			...
			// 로테이팅 캘리퍼스로 정답 계산
			Answer answer = RotatingCalipers(positions);

			// 출력에 저장
			sb.append((int) Math.round(answer.x1)).append(" ");
			sb.append((int) Math.round(answer.y1)).append(" ");
			sb.append((int) Math.round(answer.x2)).append(" ");
			sb.append((int) Math.round(answer.y2)).append("\n");
	...
	public static Answer RotatingCalipers(Position[] positions) {
		// 볼록 껍질을 이루는 점들을 찾는다
		List<Position> convexHull = getConvexHull(positions);
		int size = convexHull.size();

		int idx2 = 1;
		Answer ret = new Answer(-INF, -INF, -INF, -INF, -INF);
		...
		for (int idx1 = 0; idx1 < size; idx1++) {
			while (true) {
				...
				// 반시계 방향이 끝날 때 거리를 계산하고, 루프를 빠져나옴으로써 idx1을 옮긴다.
					...
					if (ret.maxDist < dist) {
						ret.maxDist = dist;
						ret.x1 = convexHull.get(idx1).x;
						ret.y1 = convexHull.get(idx1).y;
						ret.x2 = convexHull.get(idx2).x;
						ret.y2 = convexHull.get(idx2).y;
					}
					break;
				}
			}
