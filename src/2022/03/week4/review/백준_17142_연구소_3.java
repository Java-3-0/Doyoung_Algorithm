...
public class Main {
	...
	public static void combination(int hereIdx, int cnt) {
		...
		if (cnt == M) {
			int time = getTimeToVisitAllEmpties();
	 ...
	 * 
	 * @return 모든 빈 칸 방문 성공 시, 방문하기까지의 시간. 실패 시, INF.
	 */
	public static int getTimeToVisitAllEmpties() {
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				// 빈 칸을 방문한 경우 cnt를 세다가 모든 빈 칸의 수와 같아질 때 depth를 리턴
				if (here.getVal() == 0) {
					cnt++;
					if (cnt == totalEmpties) {
						return depth;
