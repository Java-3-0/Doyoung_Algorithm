...
public class Main {
	...
	static int[] cycleCnt = new int[MAX_V + 1];
	...
	/** adjList가 나타내는 그래프에서 각 정점의 사이클 개수를 센다 */
	...
	public static int searchEarliest(int now, int parent) {
		// 현재 정점의 발견 순서를 기록
		...
		for (int next : adjList[now]) {
			...
			if (discovered[next] == -1) {
				...
				int earliest = searchEarliest(next, now);
	...
	/** now를 포함하는 연결 요소가 선인장인지 여부를 리턴 */
	public static boolean isCactus(int now) {
		if (isVisited[now]) {
			return true;
		}
		...
		// dfs를 통해, 연결된 노드들 중 사이클이 2개 이상인 노드가 존재하는지 파악한다
		boolean ret = true;
		for (int next : adjList[now]) {
			ret &= isCactus(next);
		}
