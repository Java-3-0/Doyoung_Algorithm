...
public class Main {
	...
	static final int MAX_N = 10000, MAX_M = 10000;
	static final int EMPTY = -1;
	static final int TRUE = 1, FALSE = 0;

	static int N, M;
	static Set<Integer>[] adjList = new HashSet[2 * MAX_M + 1];
	static Stack<Integer> stack = new Stack<>();
	static int[] discovered = new int[2 * MAX_M + 1];
	static int[] sccNums = new int[2 * MAX_M + 1];
	static int[] values = new int[MAX_M + 1];
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < N; i++) {
			...
			adjList[getNodeNum(-x)].add(getNodeNum(y));
			adjList[getNodeNum(-y)].add(getNodeNum(x));
		}
