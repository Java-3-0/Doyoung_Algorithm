...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		// 보석 정보 입력
		PriorityQueue<Jewel> pqWeight = new PriorityQueue<>(N, cmpWeightAsc);
		for (int i = 0; i < N; i++) {
			st = new StringTokenizer(br.readLine(), " ");
		...
		PriorityQueue<Jewel> pqValue = new PriorityQueue<>(N, cmpValueDesc);

		// 모든 가방에 대해 수행
		long answer = 0L;
		for (int bag : bags) {
			// 이 가방의 한도보다 무게가 작은 보석들을 모두 pqValue에 넣는다.
			while (!pqWeight.isEmpty() && pqWeight.peek().weight <= bag) {
