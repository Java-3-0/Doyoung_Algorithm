...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    ...
    val mouseList = mutableListOf<Position>()
    repeat(DOMAIN) {
        val (x, y) = readLine()!!.trim().split(" ").map { it.toDouble() }
        mouseList.add(Position(y, x))
    }

    val homeList = mutableListOf<Position>()
    repeat(RANGE) {
        val (x, y) = readLine()!!.trim().split(" ").map { it.toDouble() }
        homeList.add(Position(y, x))
    }

    for (i in 0 until DOMAIN) {
        for (j in 0 until RANGE) {
            val distance = mouseList[i].getDistanceTo(homeList[j])
