...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    ...
    // 1번 위치와 n번 위치를 source 와 연결
    connectEdge(source, getInNum(1), 1, 0)
    connectEdge(source, getInNum(n), 1, 0)
    ...
    for (i in 1 .. n) {
        // 양방향 처리
        connectEdge(getInNum(i), getOutNum(i), INF, 0)
        connectEdge(getOutNum(i), getInNum(i), INF, 0)
    }
