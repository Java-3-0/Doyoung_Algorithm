...
const val MAX_LEN_A = 50
const val MAX_LEN_B = 50
const val MAX_V = MAX_LEN_A + MAX_LEN_B + 2
...
const val source = MAX_LEN_A + MAX_LEN_B + 1
const val sink = MAX_LEN_A + MAX_LEN_B + 2
...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    ...
    for (i in strA.indices) {
        connectEdge(source, i, 1, 0)
    }

    for (j in strB.indices) {
        connectEdge(j + MAX_LEN_A, sink, 1, 0)
    }
    ...
    for (i in strA.indices) {
        for (j in strB.indices) {
            val c1 = strA[i]
            val c2 = strB[j]
            val diff = if (c1 <= c2) (c2 - c1) else (('Z' - c1) + 1 + (c2 - 'A'))
            connectEdge(i, j + MAX_LEN_A, 1, diff.toLong())
        }
    }
