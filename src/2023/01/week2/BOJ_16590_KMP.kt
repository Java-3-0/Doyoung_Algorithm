...
val names = mutableListOf<HashSet<Int>>()
...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    ...
    repeat(N) {
        val set = HashSet<Int>()
        val tokens = readLine()!!.trim().split(" ")
        for (token in tokens) {
            val num = token[0].minus('A')
            set.add(num)
        }
        names.add(set)
    }
    ...
    repeat(Q) {
        ...
        if (DOMAIN > N) {
            ...
            for (from in line.indices) {
                val num = line[from].minus('A')
                for (to in 0 until N) {
                    if (names[to].contains(num)) {
                        adjList[from].add(to)
                    }
                }
            }
            ...
            val answer = if (maxMatches == DOMAIN) "YES" else "NO"
            sb.append("$answer\n")
        }
    }
