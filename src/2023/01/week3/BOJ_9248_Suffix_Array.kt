...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    val text = readLine()!!
    val suffixArray = getSuffixArray(text)
    val lcpArray = getLcpArray(text, suffixArray)

    val sb = StringBuilder()
    suffixArray.forEach { item -> sb.append("${item + 1} ") }
    sb.setLength(sb.length - 1)
    sb.append("\n")

    sb.append("x ")
    lcpArray.forEach { item -> sb.append("$item ") }
    sb.setLength(sb.length - 1)
    sb.append("\n")
