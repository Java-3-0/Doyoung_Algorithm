...
const val MAX_N = 500
const val MAX_M = MAX_N * (MAX_N - 1) / 2
...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    ...
    repeat(tests) {
        ...
        repeat(m) {
            ...
            connectEdge(a, b, INF, 1L)
            connectEdge(b, a, INF, 1L)
        }

        // 색깔 입력
        val vertexColorList = readLine()!!.trim().split(" ").map { it.toInt() }
        val coinColorList = readLine()!!.trim().split(" ").map { it.toInt() }

        // sink 와 (검은 칸, 흰 동전) 연결
        for (i in 0 until n) {
            if (vertexColorList[i] == BLACK && coinColorList[i] == WHITE) {
                connectEdge(i, sink, 1, 0L)
            }
        }

        // source 와 (흰 칸, 검은 동전) 연결
        for (i in 0 until n) {
            if (vertexColorList[i] == WHITE && coinColorList[i] == BLACK) {
                connectEdge(source, i, 1, 0L)
            }
        }
