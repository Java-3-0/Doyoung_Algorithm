...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    ...
    while (true) {
        ...
        val (jobs, students) = readLine().trim().split(" ").map { it.toInt() }
        ...
        for (i in 0 until jobs) {
            ...
            connectEdge(getJobNum(i), sink, capacity.toLong(), 0)
        }

        // student 에서 job 으로 연결
        for (studentIdx in 0 until students) {
            val st = StringTokenizer(readLine()!!.trim(), " ")
            val grade = st.nextToken().toInt()
            for (k in 0 until 4) {
                val jobIdx = st.nextToken().toInt()
                connectEdge(getStudentNum(studentIdx), getJobNum(jobIdx), 1L, -((4 * grade - k).toLong()))
            }
        }

        // source 에서 student 로 연결
        for (studentIdx in 0 until students) {
            connectEdge(source, getStudentNum(studentIdx), 1L, 0L)
        }
...
fun getStudentNum(studentIdx: Int): Int {
    return MAX_N + studentIdx
}

fun getJobNum(jobIdx: Int): Int {
    return jobIdx
}
