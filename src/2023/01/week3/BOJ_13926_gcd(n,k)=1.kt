...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    ...
    if (n == 1UL) {
        ...
        // 중복 제거
        val primeFactorSet = hashSetOf<ULong>()
        primeFactorSet.addAll(primeFactorList)

        // 오일러 phi 함수 계산
        val answer = eulerPhiFunction(n, primeFactorSet)

        println(answer)
    }
}

/** 오일러 피 함수 ([1, n] 범위의 정수 중 n과 서로소인 정수의 개수) */
fun eulerPhiFunction(n: ULong, primeFactorSet: HashSet<ULong>): ULong {
    var ret = n
    for (factor in primeFactorSet) {
        ret /= factor
        ret *= (factor - 1UL)
    }
