...
fun getInVertex(idx: Int): Int {
    return 2 * idx
}

fun getOutVertex(idx: Int): Int {
    return 2 * idx + 1
}
...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    ...
    repeat(m) {
        ...
        connectEdge(getOutVertex(a), getInVertex(b), 1L, cost)
        connectEdge(getOutVertex(b), getInVertex(a), 1L, cost)
    }

    val (home, target) = readLine().trim().split(" ").map { it.toInt() - 1 }

    // 정점 분할 내부 간선 연결
    for (i in 0 until n) {
        val capacity = if (i == home || i == target) 2L else 1L
        connectEdge(getInVertex(i), getOutVertex(i), capacity, 0L)
    }
    source = getInVertex(home)
    sink = getOutVertex(target)

    val mcmf = getMinCostMaxFlow()
    if (mcmf.maxFlow == 2L) {
        println(mcmf.minCost)
    } else {
        println(-1)
    }
}
