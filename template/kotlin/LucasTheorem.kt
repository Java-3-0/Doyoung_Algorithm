...
fun main(): Unit = with(System.`in`.bufferedReader()) {
    val (n, k, mod) = readLine()!!.trim().split(" ").map { it.toLong() }
    val answer = getCombinationByLucas(n, k, mod)
    println(answer)
}


/** 뤼카의 정리를 이용하여 (nCk % mod) 구해서 리턴 */
fun getCombinationByLucas(n: Long, k: Long, mod: Long): Long {
    ...
    /** 모듈러 곱셈 */
    fun modMult(a: Long, b: Long): Long {
        return (a * b) % mod
    }
    ...
    fun getSmallCombination(n: Int, k: Int): Long {
        ...
        var ret = modAdd(getSmallCombination(n - 1, k - 1), getSmallCombination(n - 1, k))
        cache[n][k] = ret
        return ret
    }
